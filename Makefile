...
MTHEME = \
	$(wildcard eshanized-live/*.png) \
	eshanized-live/theme.txt \
	eshanized-live/*.pf2

MICONS= $(wildcard eshanized-live/icons/*.png)
...
install_snigdha:
	install -dm0755 $(DESTDIR)$(PREFIX)/share/grub/themes/eshanized-live
	install -m0644 ${MTHEME} $(DESTDIR)$(PREFIX)/share/grub/themes/eshanized-live

	install -dm0755 $(DESTDIR)$(PREFIX)/share/grub/themes/eshanized-live/icons
	install -m0644 ${MICONS} $(DESTDIR)$(PREFIX)/share/grub/themes/eshanized-live/icons

uninstall_snigdha:
	for f in ${MTHEME}; do rm -f $(DESTDIR)$(PREFIX)/share/grub/theme/eshanized-live/$$f; done
	for f in ${MICONS}; do rm -f $(DESTDIR)$(PREFIX)/share/grub/theme/eshanized-live/icons/$$f; done
